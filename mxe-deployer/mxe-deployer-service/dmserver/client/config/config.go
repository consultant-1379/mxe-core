...
var MXE_CLI_VERSION string

const (
	TOKEN_ADDITIONAL_VALIDITY_SEC = 40
)

// DefaultConfigDir returns the local configuration path for settings such as cached authentication tokens.
func DefaultConfigDir(cliName string) (string, error) {
	...
	return path.Join(homeDir, fmt.Sprintf(".%s", cliName)), nil
}

// DefaultLocalConfigPath returns the local configuration path for settings such as cached authentication tokens.
func DefaultLocalConfigPath(cliName string) (string, error) {
	dir, err := DefaultConfigDir(cliName)
	if err != nil {
		return "", err
	}
