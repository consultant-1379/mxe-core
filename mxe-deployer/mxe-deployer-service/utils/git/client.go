...
type Client interface {
	...
	//added for DM
	//TODO consider creating DMClient with these functions so that argocd git client can be reused as-is
	AddFiles(fileNames []string) error
	Commit(commitAuthorName string, commitAuthorEmail string, message string) error
	Remove(fileNames []string) error
	Push(revision string) error
	RemoveFiles(appName string, sourcePath string, revision string, commitAuthorName string, commitAuthorEmail string) error

	//Wrappers
	Initialize(revision string, msg string) error
	Checkin(dir string, commitMessage string, revision string, commitAuthorName string, commitAuthorEmail string) error
}
...
func (m *nativeGitClient) Fetch() error {
	err := m.runCredentialedCmd("git", "fetch", "origin", "--tags", "--force", "--depth", "1", "--prune", "--prune-tags")
...
func (m *nativeGitClient) AddFiles(files []string) error {
	return m.runCredentialedCmd("git", "add", strings.Join(files, " "))

}

func (m *nativeGitClient) Remove(files []string) error {
	return m.runCredentialedCmd("git", "rm", "-r", strings.Join(files, " "))
}

func (m *nativeGitClient) Commit(commitAuthorName string, commitAuthorEmail string, message string) error {
	//author := "user.name=" + authorName
	//email := "user.email=" + authorEmail
	...
	//commented because of https://github.com/src-d/go-git/issues/1268
	/*
...
func (m *nativeGitClient) Push(revision string) error {
	r, err := git.PlainOpen(m.root)
	if err != nil {
		return err
	}
...
/* func Push(revision string) error {
	return m.runCredentialedCmd("git", "push", "--porcelain", "--verbose")
}
*/
