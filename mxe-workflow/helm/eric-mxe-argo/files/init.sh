...
connectToMinio() {
  ...
  echo "Service Mesh enabled: $SIP_MTLS_ENABLED"
  if [ "${SIP_MTLS_ENABLED}" = true ] ; then
    MK_CMD="mkdir -p $HOME/.mc/certs/CAs"
    $MK_CMD
    CP_CMD="cp /tmp/certificates/trusted/* $HOME/.mc/certs/CAs/."
    $CP_CMD  
  fi
