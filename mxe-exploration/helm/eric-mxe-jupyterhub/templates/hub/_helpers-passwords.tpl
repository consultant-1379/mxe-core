...
*/}}
{{- define "eric-mxe-jupyterhub.randHex" -}}
    {{- $result := "" }}
    {{- range $i := until . }}
...
{{- define "eric-mxe-jupyterhub.hub.config.ConfigurableHTTPProxy.auth_token" -}}
    ...
    {{- else }}
        {{- $k8s_state := lookup "v1" "Secret" .Release.Namespace (include "eric-mxe-jupyterhub.hub-config-name" . | quote) | default (dict "data" (dict)) }}
...
{{- define "eric-mxe-jupyterhub.hub.config.JupyterHub.cookie_secret" -}}
    ...
    {{- else }}
        ...
        {{- else }}
            {{- include "eric-mxe-jupyterhub.randHex" 64 }}
        {{- end }}
    {{- end }}
{{- end }}

{{- define "eric-mxe-jupyterhub.hub.config.CryptKeeper.keys" -}}
...
{{- define "eric-mxe-jupyterhub.hub.services.get_api_token" -}}
    {{- $_ := index . 0 }}
    {{- $service_key := index . 1 }}
    ...
    {{- else }}
        {{- $k8s_state := lookup "v1" "Secret" $_.Release.Namespace (include "eric-mxe-jupyterhub.hub-config-name" $_) | default (dict "data" (dict)) }}
