{{- /*
  ...
  {{ include "eric-mxe-jupyterhub.commonLabels" . }}
  ...
  {{- include "eric-mxe-jupyterhub.matchLabels" $_ | nindent 6 }}
  ...
  kind: Deployment
  metadata:
    name: {{ include "eric-mxe-jupyterhub.autohttps.fullname" . }}
    labels:
  ...
  spec:
    ...
    template:
      metadata:
        labels:
          {{- include "eric-mxe-jupyterhub.labels" $_ | nindent 8 }}
          hub.jupyter.org/network-access-proxy-http: "true"
  ```

  NOTE:
    The "eric-mxe-jupyterhub.matchLabels" and "eric-mxe-jupyterhub.labels" is passed an augmented
...
{{- /*
  jupyterhub.appLabel:
    Used by "eric-mxe-jupyterhub.additionalLabels".
*/}}
{{- define "eric-mxe-jupyterhub.appLabel" -}}
...
*/}}
{{- define "eric-mxe-jupyterhub.componentLabel" -}}
{{- $file := .Template.Name | base | trimSuffix ".yaml" -}}
{{- $parent := .Template.Name | dir | base | trimPrefix "templates" -}}
{{- $componentBody := .componentLabel | default $parent | default $file -}}
{{- $component := print (.componentPrefix | default "") $componentBody (.componentSuffix | default "") -}}
{{ $component }}
{{- end }}
...
{{- /*
  jupyterhub.commonLabels:
    Foundation for "eric-mxe-jupyterhub.additionalLabels".
    Provides labels: app, release, (chart and heritage).
*/}}
{{- define "eric-mxe-jupyterhub.commonLabels" -}}
app: {{ .appLabel | default (include "eric-mxe-jupyterhub.appLabel" .) }}
...
*/}}
{{- define "eric-mxe-jupyterhub.additionalLabels" -}}
component: {{ include "eric-mxe-jupyterhub.componentLabel" . }}
{{ include "eric-mxe-jupyterhub.commonLabels" . }}
{{- end }}
...
*/}}
{{- define "eric-mxe-jupyterhub.matchLabels" -}}
{{- $_ := merge (dict "matchLabels" true) . -}}
{{ include "eric-mxe-jupyterhub.additionalLabels" $_ }}
{{- end }}
...
*/}}
{{- define "eric-mxe-jupyterhub.imagePullSecrets" -}}
...
{{- $__ := set $_ "list" (append $_.list (include "eric-mxe-jupyterhub.image-pull-secret.fullname" .root) | uniq) }}
{{- end }}
...
*/}}
{{- define "eric-mxe-jupyterhub.singleuser.resources" -}}
...
*/}}
{{- define "eric-mxe-jupyterhub.extraEnv" -}}
{{- include "eric-mxe-jupyterhub.extraEnv.withTrailingNewLine" . | trimSuffix "\n" }}
{{- end }}

{{- define "eric-mxe-jupyterhub.extraEnv.withTrailingNewLine" -}}
{{- if . }}
...
{{- /*
  jupyterhub.extraFiles.data:
    ...
    binaryData entries.
*/}}
{{- define "eric-mxe-jupyterhub.extraFiles.data.withNewLineSuffix" -}}
    {{- range $file_key, $file_details := . }}
        {{- include "eric-mxe-jupyterhub.extraFiles.validate-file" (list $file_key $file_details) }}
        {{- if $file_details.binaryData }}
            {{- $file_key | quote }}: {{ $file_details.binaryData | nospace | quote }}{{ println }}
        {{- end }}
    {{- end }}
{{- end }}
{{- define "eric-mxe-jupyterhub.extraFiles.data" -}}
    {{- include "eric-mxe-jupyterhub.extraFiles.data.withNewLineSuffix" . | trimSuffix "\n" }}
{{- end }}
...
*/}}
{{- define "eric-mxe-jupyterhub.extraFiles.stringData.withNewLineSuffix" -}}
    {{- range $file_key, $file_details := . }}
        {{- include "eric-mxe-jupyterhub.extraFiles.validate-file" (list $file_key $file_details) }}
    ...
    {{- end }}
{{- end }}
{{- define "eric-mxe-jupyterhub.extraFiles.stringData" -}}
    {{- include "eric-mxe-jupyterhub.extraFiles.stringData.withNewLineSuffix" . | trimSuffix "\n" }}
{{- end }}

{{- define "eric-mxe-jupyterhub.extraFiles.validate-file" -}}
    {{- $file_key := index . 0 }}
