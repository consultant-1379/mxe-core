...
def _load_config():
    ...
    for source in ("secret/values.yaml", "existing-secret/values.yaml"):
        path = f"/run/secrets/usr/local/etc/jupyterhub/{source}"
...
def _get_config_value(key):
    ...
    path = f"/run/secrets/usr/local/etc/jupyterhub/config/{key}"
    if os.path.exists(path):
        with open(path) as f:
            return f.read()
    else:
...
def get_secret_value(key, default="never-explicitly-set"):
    ...
    for source in ("existing-secret", "secret"):
        path = f"/run/secrets/usr/local/etc/jupyterhub/{source}/{key}"
        if os.path.exists(path):
            with open(path) as f:
                return f.read()
