...
## Change1: MxE specific change. This needs to be commented out:
##c.ConfigurableHTTPProxy.api_url = (
## f'http://{get_name("proxy-api")}:{get_name_env("proxy-api", "_SERVICE_PORT")}'
##)
## Change2: And replaced with this line from the Ericsson gitlab gaia jupyterhub helm project
## gaia/services/helm/gaia-jupyterhub-helm/-/blob/develop/gaia-jupyterhub/files/hub/jupyterhub_config.py#L24
...
#
# This is needed to bypass the CORS issue which prevented nb from connecting
# to kernel. This is needed at two places, tornado setting and spanwer args
# This is for torando settings. See below for spawner args
#
# See https://github.com/jupyterhub/jupyterhub/issues/1087#issuecomment-398762314
#
origin = "*"
...
#
# MxE specific change : This needs to be commented out
# c.JupyterHub.hub_connect_url = (
#    f'http://{get_name("hub")}:{get_name_env("hub", "_SERVICE_PORT")}'
# )
# and replaced with below
c.JupyterHub.hub_connect_url = "http://{}:{}".format(
    os.environ["ERIC_MXE_JUPYTERHUB_HUB_SERVICE_SERVICE_HOST"],
    int(os.environ["ERIC_MXE_JUPYTERHUB_HUB_SERVICE_SERVICE_PORT"]),
)
...
if extra_files:
    ...
    for file_key, file_details in extra_files.items():
        ...
        items.append(item)
    volume["secret"] = {
        "secretName": get_name("singleuser-extrafile-secret"),
        "items": items,
    }
...
#Inject pypi server configmap
c.KubeSpawner.volumes.extend([
    {
        "name": "pypi-conf",
        "configMap": {
            "name": get_name('hub-config'),
            "items":[
                {
                    "key": "pipConf",
                    "path": "pip.conf"
...
c.KubeSpawner.volume_mounts.extend([
    {
        "name": "pypi-conf",
        "mountPath": get_config("singleuser.storage.homeMountPath") + "/.pip"
    }
])
...
if get_config("cull.enabled", False):
    ...
    cull_cmd = ["python3.11", "-m", "jupyterhub_idle_culler"]
...
if cloud_metadata.get("blockWithIptables") == True:
    ...
    pullpolicy = get_config("singleuser.image.pullPolicy")
    ip_block_container = client.V1Container(
        name="block-cloud-metadata",
        image=f"{network_tools_image_name}:{network_tools_image_tag}",
        image_pull_policy=f"{pullpolicy}",
        command=[
            "iptables",
            "-A",
            "OUTPUT",
...
#
# This is needed to bypass the CORS issue which prevented nb from connecting
# to kernel. This is needed at two placed: tornado settings and spawner args
# This is for spawner args
#
# See https://github.com/jupyterhub/jupyterhub/issues/1087#issuecomment-398762314
#
c.Spawner.args = ["--NotebookApp.allow_origin={0}".format(origin)]

# load /run/secrets/usr/local/etc/jupyterhub/jupyterhub_config.d config files
config_dir = "/run/secrets/usr/local/etc/jupyterhub/jupyterhub_config.d"
