...
@Configuration
@Profile("production")
public class TrainingPackageRepositoryConfig {
    @Bean
    public MinioBucketService trainingPackageRepository(final MinioProperties minioProperties,
            final MinioClient.Factory minioClientFactory) throws Exception {
        return new MinioBucketService(new URL(minioProperties.getUrl()), minioProperties.getTrainingPackage(),
                minioClientFactory);
    }
}
